...
function unsplashRandom(query: string, opts: { w: number; h: number; sig: number }) {
  // Uses Unsplash Source endpoint; sig makes each URL unique (reduces repetition).
  const q = encodeURIComponent(query);
  return `https://source.unsplash.com/${opts.w}x${opts.h}/?${q}&sig=${opts.sig}`;
}

function genDialogue(theme: TrendWebtoon['theme'], synopsis: string, idx: number) {
  const p = idx / 70;
...
function genCutTheme(theme: TrendWebtoon['theme'], idx: number) {
  const p = idx / 70;
  const beat = p < 0.15 ? '도입' : p < 0.35 ? '긴장' : p < 0.6 ? '전개' : p < 0.8 ? '충돌' : '여운';
  ...
  return beat === '도입' ? '심연의 기척' : beat === '긴장' ? '봉인 해제' : beat === '전개' ? '룬의 속삭임' : beat === '충돌' ? '수호' : '잔향';
}
...
const WEBTOONS: TrendWebtoon[] = [
  {
    ...
    query: 'cyberpunk neon alley rain seoul night',
    isHot: true,
    isNew: true,
  },
  {
    ...
    query: 'korean cafe rain umbrella bokeh night',
    isNew: true,
  },
  {
    id: 'abyss-keeper',
    ...
    query: 'dark fantasy ruins fog rune gate cinematic',
    isHot: true,
  },
];

async function seedTrendingUnsplash() {
  console.log('Seeding TRENDING webtoons (approved) with Unsplash-style images + unique cuts + dialogue...');

  for (const w of WEBTOONS) {
    const thumb = unsplashRandom(w.query, { w: 400, h: 600, sig: 10 + w.id.length });
    const banner = unsplashRandom(w.query, { w: 1920, h: 1080, sig: 11 + w.id.length });
    ...
    for (let ep = 1; ep <= 2; ep++) {
      const cuts = Array.from({ length: 70 }).map((_, i) => {
        const sig = ep * 1000 + i * 13 + (w.id.length % 97);
        return {
          imageUrl: unsplashRandom(w.query, { w: 800, h: 1200, sig }),
