...
// High Quality Dialogues (70 cuts each)
...
function generateUrl(basePrompt: string, dialogue: string, seed: number) {
  // Use the Korean dialogue directly to guide the AI's composition
  const prompt = `${basePrompt}, scene depicting: ${dialogue}, no text`;
  return `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=800&height=1200&nologo=true&seed=${seed}`;
}

async function seed() {
  console.log("1. Wiping ALL webtoons to clear 404 Unsplash URLs...");
  ...
  console.log("2. Restoring 5 Original Webtoons with PERFECTLY MATCHED AI URLs...");

  for (const webtoon of WEBTOONS) {
    const { id, basePrompt, ...data } = webtoon;
    const baseSeed = webtoon.title.length * 1000;

    await setDoc(doc(db, 'webtoons', id), {
      ...data,
      thumbnail: generateUrl(basePrompt, "main cover title image", baseSeed),
      banner: generateUrl(basePrompt, "horizontal banner landscape", baseSeed + 1) + "&width=1920&height=1080",
      status: 'approved',
      isNew: true,
      episodeCount: 1,
    ...
    const cuts = [];
    for (let i = 0; i < 70; i++) {
      const dialogue = getDialogue(data.theme, i);
      cuts.push({
        imageUrl: generateUrl(basePrompt, dialogue, baseSeed + 100 + i),
        dialogue: dialogue
      });
    }
    ...
    console.log(`[Success] Restored ${data.title} (70 cuts with perfectly matched dialogue prompts)`);
  }

  console.log("All Webtoons perfectly restored with matching AI URLs! No more 404s!");
  process.exit(0);
}
