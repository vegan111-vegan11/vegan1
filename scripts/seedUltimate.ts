...
async function seed() {
  ...
  console.log("2. Restoring 5 Original Webtoons with 100% EXPLICIT ENGLISH PROMPTS...");
  ...
  for (const webtoon of WEBTOONS) {
    ...
    await setDoc(doc(db, 'webtoons', id), {
      ...meta,
      thumbnail: generateUrl(basePrompt, data[0].en, baseSeed),
      banner: generateUrl(basePrompt, data[7].en, baseSeed + 1) + "&width=1920&height=1080",
      status: 'approved',
      isNew: true,
      episodeCount: 1,
    ...
    const cuts = [];
    // We will generate 70 cuts. Since we have 56 unique data points, we'll just loop them safely.
    for (let i = 0; i < 70; i++) {
      const line = data[i % data.length];
      cuts.push({
        imageUrl: generateUrl(basePrompt, line.en, baseSeed + 100 + i),
        dialogue: line.kr
      });
    }
    ...
    console.log(`[Success] Restored ${meta.title} (70 cuts with PERFECT English prompts)`);
  }

  console.log("All Webtoons perfectly restored! Prompts are 100% explicit English descriptions!");
  process.exit(0);
}
