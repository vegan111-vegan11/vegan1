...
async function seedDB() {
  console.log("Seeding Database with perfectly matched URLs...");
  ...
  await setDoc(doc(db, 'webtoons', webtoonId), {
    ...
    rating: 5.0,
    theme: '로맨스',
    description: '대사와 이미지가 100% 완벽하게 일치하는 초고퀄리티 스토리 연출판입니다.',
    status: 'approved',
    isNew: true,
    thumbnail: `https://image.pollinations.ai/prompt/korean%20webtoon%20style,%20romance,%20raining%20city%20street%20moody%20sad%20girl%20walking%20with%20umbrella,%20cinematic%20lighting,%20highly%20detailed,%20highly%20aesthetic?width=400&height=600&nologo=true&seed=501`,
    banner: `https://image.pollinations.ai/prompt/korean%20webtoon%20style,%20romance,%20soft%20glowing%20light%20connecting%20two%20people%20in%20cafe%20romantic,%20cinematic%20lighting,%20highly%20detailed,%20highly%20aesthetic?width=1920&height=1080&nologo=true&seed=508`,
    episodeCount: 1,
  ...
  const cuts = cutsData.map((data, i) => {
    ...
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=800&height=1200&nologo=true&seed=${500 + i + 1}`;
  ...
  console.log(`[Success] Seeded webtoon with perfectly matched AI URL prompts.`);

  // Optional Cache Warming: 
  // We fire fetch requests to the URLs and just ignore the response
  // This causes Pollinations to generate the image NOW, so when the user scrolls, Cloudflare serves it instantly.
  console.log('Warming up Cloudflare Cache for 70 cuts in the background...');
  cuts.forEach(c => {
    fetch(c.imageUrl).catch(() => {});
  });
